...
def lst2exp(chr_num, lst, idx, schr, warn_list):
    out_dict = {}
    chr_num_tmp = 46
    if lst:
        ...
        for ch, exp in exp_list:
            if exp:
                ...
                warn_list.append(f'illegle character:{lst}')
    ...
    if chr_num:
        if chr_num != str(chr_num_tmp):
            ...
            warn_list.append(f'{idx}:{chr_num}:NOT_EQUAL_!!!:{out_dict}')
    return out_dict, warn_list
...
def get_note(schr, exp_dict):
    ...
    if not schr:
        ...
        note = '不推荐移植'
        out = 'XXY综合征'
    elif schr and schr.upper() == 'XXX':
        note = '不推荐移植'
        out = 'XXX综合征'
    elif schr and schr.upper() == 'XYY':
        note = '不推荐移植'
        out = 'XYY综合征'
    elif schr and schr.upper() == 'OO':
        note = '不推荐移植'
        out = '性染色体缺失'
        ...
        for r in res_chr:
            if r:
                if pattern_chr.match(r):
                    ...
                    total_warning.append(f'{idx}:{"not match"}')
                    total_lst.append([r])
        # if total_lst:
